    ...
    <div className="mt-6 min-w-0 space-y-4">
      <Card className="print:border-none print:shadow-none">
        <div className="mx-auto min-w-0 max-w-xl text-center">
          <p className="break-words text-xs font-medium uppercase tracking-[0.35em] text-[var(--color-muted)]">
            {tenant.name}
          </p>
          ...
          <div className="mx-auto mt-8 flex w-full max-w-[18rem] rounded-2xl border border-[var(--color-border)] bg-white p-3 shadow-sm sm:w-fit sm:max-w-none sm:p-4">
            {qr ? (
              <img
                ...
                className="aspect-square h-auto w-full sm:h-72 sm:w-72"
              />
            ) : (
              <div className="grid aspect-square w-full place-items-center text-sm text-[var(--color-muted)] sm:h-72 sm:w-72">
                Generando QR...
              </div>
            )}
          </div>
      ...
      <div className="flex flex-col gap-2 print:hidden sm:flex-row sm:flex-wrap">
        <Button
          ...
          variant="secondary"
          onClick={copyLink}
          className="w-full transition hover:-translate-y-0.5 hover:shadow-sm sm:w-auto"
        >
